...
- Rules that read across sessions, across users or across a missing ledger: K9 counts a second connection only within one app session and ignores attempts that failed; K2 looks for the connection a call waited for in its own session only; A7 no longer takes a products read Google refused as evidence of a sale; I5 no longer counts a client_response as a write, yields to a lookup that matched nobody (that is H1's or H3's finding), and speaks at lower confidence when the file has no ledger row at all, since the ledger may have been left out; K4 does not read a second buyer as a re-purchase when the accounts differ.
