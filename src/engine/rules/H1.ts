...
export const H1 = defineRule({
  ...
  detects:
    'A resource whose externalAccountIdentifiers is null, and a ledger lookup for the token that matched no user. Set an obfuscated account id at purchase time; it is the only link when the verify call never ran.',
  run(tl) {
    const hits = [];
    for (const [token, events] of tl.byToken) {
      const read = events.find((e) => isOkGet(e) && e.externalAccountIdentifiers === null);
      if (!read) continue;
      ...
      hits.push({
        ...
        mechanism: `The resource for ${token} (#${read.i}) carries no externalAccountIdentifiers and the lookup at #${lookup.i} matched no user, so whatever this notification announced was applied to nobody.`,
