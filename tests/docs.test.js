...
import { GOOGLE_RULES, googleRule, READ_ON, READ_ON_DEVICE } from '../dist/google/docs.js';

// The 89 rule ids of BRIEF section 5, by group.
...
// The ids added in the second sweep, read on the later date.
export const SECOND_SWEEP = new Set([
  'A7', 'B10', 'B11', 'C8', 'D11', 'E7', 'E8', 'F5', 'F6', 'G7', 'I6', 'J5',
  'K1', 'K2', 'K3', 'K4', 'K5', 'K6', 'K7', 'K8', 'K9', 'K10', 'K11', 'K12', 'K13', 'K14',
]);

test('there are 89 rule ids and each has a Google fact', () => {
  assert.equal(RULE_IDS.length, 89);
...
test('the read date is the audit sweep date until the next re-read', () => {
  ...
  for (const id of RULE_IDS) {
    const expected = SECOND_SWEEP.has(id) ? READ_ON_DEVICE : READ_ON;
