...
export function makeComposites(b) {
  ...
  composites['01-week-upgrade-refund-duplicate'] = timeline([
    ...
    get('tok_b2', at(U, 10 * MIN + SEC), { expiry: UE, productId: 'plus_monthly', linked: 'tok_a1' }),
    ...
    get('tok_b2', at(S, 6 * DAY + SEC), { expiry: UE, productId: 'plus_monthly', linked: 'tok_a1' }),
  ...
  composites['03-support-ticket'] = timeline(
    [
      ...opening('tok_a1', S, { expiry: E, messageId: 'm-1' }),
      purchase('tok_c1', at(S, HOUR), { productId: 'coins_100', extra: { quantity: 3 } }),
      productGet('tok_c1', at(S, HOUR + 2 * SEC), { productId: 'coins_100', purchaseState: 'PURCHASED', extra: { quantity: 3 } }),
      grant('tok_c1', at(S, HOUR + 3 * SEC), { tier: 'coins' }),
      ...
      productGet('tok_c1', at(S, 3 * DAY + SEC), { productId: 'coins_100', purchaseState: 'PURCHASED', ack: 'ACKNOWLEDGEMENT_STATE_ACKNOWLEDGED', extra: { quantity: 3, refundableQuantity: 2 } }),
      ledger('revoke', 'tok_c1', at(S, 3 * DAY + 2 * SEC), { messageId: 'm-3' }),
      rtdn(20, 'tok_a1', at(S, 4 * DAY), { messageId: 'm-4' }),
      support(at(S, 5 * DAY), 'user asks why the coins and the subscription both vanished'),
    ],
